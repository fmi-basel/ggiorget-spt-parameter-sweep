...
files = pd.read_csv('/tungstenfs/scratch/ggiorget/nessim/snakemake/benchmark_spot_detection/LoG/results/results.txt',header=None)
files = files[files.columns[0]].values

# for l,i in enumerate(files):
#     files[l]=i.replace('clean','noisy')

num = [int(re.findall(r'\d+',x)[2]) for x in files]
num = sorted(num)
...
def build_condition(df_exp,num,df_gt):
    ...
    for i in num: #loop over images
        for j in [0,-1]: #loop over frames
            ...
            test_astro = test_astro[test_astro.frame == j]

            test_gt = df_gt[df_gt.image == i ]
            test_gt = test_gt[test_gt.frame == j]              
